...
class SystemAdmin extends AuthController
{
    ...
    public function create()
    {
        ...
        $f[] = Form::select('roles','管理员身份')->setOptions(function ()use($admin){
                    $list = SystemRole::getRole(bcadd($admin->level,1,0),'id,role_name',$admin->business);
                    $options = [];
        ...
        if($admin->business==0){
            $f[]=Form::input('business','商家标识')->placeholder('如果是商家，请填写商家唯一标识，否则默认不填。由字母或者数字组成')
            ->maxlength(16);
        }else{
            $f[]=Form::input('business','')->type('hidden')->value($admin->business);
        }
    ...
    public function save(Request $request)
    {
        ...
        if(count(array_filter($data['roles']))<1) return Json::fail('请选择至少一个管理员身份');
    ...
    public function edit($id)
    {
        ...
        $f[] = Form::select('roles','管理员身份',explode(',',$admin->roles))->setOptions(function ()use($admin){
            $list = SystemRole::getRole($admin->level,'id,role_name',$this->adminInfo->business);
            $options = [];
            foreach ($list as $id=>$roleName){
                $options[] = ['label'=>$roleName,'value'=>$id];
            }
