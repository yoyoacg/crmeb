...
//                if($v['upload_type'] == 1 || $v['upload_type'] == 3) $list[$k]['value'] = explode(',',$v['value']);
   ...
   public function index(){
       $where = Util::getMore([
           ['status',''],
           ['title',''],
           ['business',$this->adminInfo->business],
       ],$this->request);
//       $value = ConfigTabModel::where('business',$this->adminInfo->business)->limit(1)->value('id');
//       if(!$value){
//           $list = ConfigTabModel::where('business','0')->column('*');
//       }
